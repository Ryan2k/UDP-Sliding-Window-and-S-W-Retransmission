 ...
 * This file was given by Dr. Peng and is used to
 * 1. Instatiate a sock - a UdpSocket object
 * 2. Allocates a 1460 byte message (character array)
 * 3. Evaluates the peformacne of UDP point to point communication using 3 different test cases
 *    - These test cases are explained in udphw3.cpp
 */
...
#define PORT "42648"     // my UDP port, note that it is not an integer, this is the last 5 digits of my UW ID Number
...
int main( int argc, char *argv[] ) {
  ...
  cerr << "   4: sliding window with 2 cases" << endl;
  cerr << "--> ";
  cin >> testNumber;
  ...
  if ( myPart == CLIENT ) {
    ...
    case 3:
      ...
      break;
      /**
       * This case is the only modification from the original hw3.cpp
       * Simply tests the slididng window algorithm with only sizes 1 and 30
       * However, It calls it with the udphw3case4.cpp slididng window algorithms
       * Where the client side code randomly drops N% packets of the time
       * Where N is any integer between 0 and 10.
       */
    case 4:
        cout << "Case 4" << endl;
        for (int windowSize = 1; windowSize <= 30; windowSize += 29) {
            timer.start();
            ...
            double dropPercentage = 20000 / retransmits;
            cerr << "Percentage of Packets Dropped = " << dropPercentage << endl;
        }
    default:
